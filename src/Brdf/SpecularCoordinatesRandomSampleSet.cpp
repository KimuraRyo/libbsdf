                                                   ...
                                                   float                    weight3)
{
    ...
    for (int spThIndex = 0; spThIndex < brdf->getNumSpecTheta(); ++spThIndex) {
        float angleInput3 = brdf->getInTheta(inThIndex) / SpecularCoordinateSystem::MAX_ANGLE0
                          + brdf->getSpecTheta(spThIndex) / SpecularCoordinateSystem::MAX_ANGLE2;
        angleInput3 /= 2.0;
        ...
        for (int spPhIndex = 0; spPhIndex < brdf->getNumSpecPhi(); ++spPhIndex) {
            ...
            if (it != sampleMap_.end()) {
                ...
                w3 = hermiteInterpolation3(weight3, 10.0f * weight3, angleInput3);
